...
- **Cassandra & SQL Datetime Localization**: Refactored the row mappers in both Cassandra and MSSQL repositories to read mapping / machine local timezones (defaulting to `"Europe/Zurich"`). Naive database datetimes (which represent UTC times in the database) are explicitly parsed as UTC first and then shifted to the target local timezone to format them with the correct offset (e.g. `+02:00` or `+01:00` depending on DST). Timezone-offset database fields (`DatetimeOffsetn`) are similarly converted to the target timezone before serialization, ensuring complete parity with C# and correct transition handling.
